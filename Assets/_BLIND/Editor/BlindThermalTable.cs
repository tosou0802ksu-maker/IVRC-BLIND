...
namespace BLIND.EditorTools
{
    ...
    public static class BlindThermalTable
    {
        ...
        public static readonly Temp[] All =
        {
            ...
            // ⚠️ 落とし穴フロアの踏み板だけは、周囲の床(dim=0.022 ＝ ほぼ真っ黒)と
            // 同じにしてはいけない。穴の内壁が dim=1.0 でくっきり見えるようになった結果、
            // **足場が真っ黒で穴だけが宙に浮いて見える**状態になっていた。
            // 「浮いたパネルに見える」の一因はこれ。立っている面が見えないと
            // 穴との位置関係が読めず、深さの段も奥行きとして解釈されない。
            //
            // dim は明るさと同時に「見える距離」も決める（range = lerp(5,200,dim²)、
            // range*0.45 から溶け始める）。0.13 なら 約3.7m から溶けて 8.3m で消える。
            // つまり**足元と一歩先だけが見えて、部屋の形は依然として読めない**ので、
            // 「空間の形はエコロケ役の担当」という切り分けは保たれる。
            // ⚠️ 温度は普通の床(FloorStone)と同じ 15℃ のままにすること。
            // 一度 34℃(暗い緑)にしたら「なぜ床の色が変わっているのか」と指摘された。
            // 落とし穴部屋だけ床の色が違うと、そこだけ別の床材に見えて嘘になる。
            // 変えていいのは **明るさ(dim)だけ**。色は他の部屋の床と揃える。
            new Temp("PitDeck",     15.0f, 0.110f, "落とし穴の安全な踏み板。色は普通の床と同じで、明るさだけ上げて足元が見えるようにする"),
            // 落とし穴は「冷たい」で見せる。
            //
            ...
            // --- 落とし穴の内側 ---
            // 開口に蓋（水平な板）は張らない。**縦坑の内壁そのものを深さで温度分けする。**
            //
            // 蓋を張ると、浅く置けば「浮いたパネル」、深く置けば「遠くで消える」に
            // なるうえ、そもそも穴の中が見えない。内壁を3段に分けると、
            // 覗き込んだときに 水色 → 青 → 濃紫 と輪が重なって見え、
            // それ自体が「どこまで深いか」を伝える。
            // 冷気は下に溜まるので、下ほど冷たいのは現象としても正しい。
            // 段どうしの見た目の差は温度差ではなく **ランプ上の距離** で決まる。
            // h = (T/70)^0.65 なので 12/7/3℃ は h=0.32/0.22/0.13。
            // 下2段が近すぎて真上から見ると1色に潰れていたので、底を 1.0℃(h=0.06)
            // まで下げて濃紫に振り、水色→青→濃紫の3段がはっきり分かれるようにした。
            new Temp("PitRim",      14.0f, 1.000f, 0f, DisplayMax,
                     "落とし穴の入口まわりの内壁(0〜0.40m)。水色。ここが一番よく見える"),
            new Temp("PitWall",      7.0f, 1.000f, 0f, DisplayMax,
                     "落とし穴の中ほどの内壁(0.40〜1.60m)。青"),
            new Temp("PitVoid",      1.0f, 1.000f, 0f, DisplayMax,
                     "落とし穴の深部と底(1.60m〜)。濃い青紫。冷気が溜まる一番深い所"),
            ...
            // ⚠️ 鍵は「明るくする」だけでは足りない。**白飛びさせること。**
            //   `Metal`(16℃) は論外として、30℃(緑)でも試したが、
            //   11cm の物は 4.5m 先で 15px ほどにしかならず、
            //   緑の点は暗い画面の中で見分けが付かなかった（実測）。
            //   白飛びが始まる 64.1℃ を超えさせると、小さくても
            //   **真っ黒な部屋に白い点が1つ**という状態になって必ず見つかる。
            //   持ち帰る物を1人しか見つけられないと、その1人の指示待ちになってしまう。
            new Temp("Key",         66.0f, 1.000f, "持ち帰る鍵。小さいので白飛びさせて「黒い部屋の白い点」にする"),

            // ⚠️ 施錠された扉を `Metal`(16℃) で塗ると**壁(18℃)より冷たくなって溝に見える**。
            //   「進めない板がそこにある」と読ませたいので、壁より少しだけ暖かくする。
            //   差を大きくし過ぎると今度は熱源に見えて「調べる物」と誤解されるため、
            //   2.5℃ だけ上げて「壁ではない、別の面」に留める。
            new Temp("Gate",        20.5f, 0.620f, "鍵で開く扉。壁(18℃)より少し暖かくして「壁ではない板」と読ませる"),

            // ⚠️ room6 の「運ぶアヒル」。周りの 60体(Prop 19℃ dim 0.095)から
            //    **一目で拾えないと成立しない**。サーモ役が3体を名指しできなければ、
            //    過去人は 61体の中からしらみつぶしに探すことになる。
            //    44℃ は橙。白飛び(64℃)まで上げないのは、鍵と違って
            //    「どの向きに置いてあるか」も見せたいから（白飛びすると形が潰れる）。
            new Temp("DuckHot",     44.0f, 0.950f, "room6 で運ぶアヒル3体。60体の中から一目で拾えるよう橙にする"),
            new Temp("DuckSlot",    33.0f, 0.900f, "アヒルを置く受け皿。3役とも「ここへ置く」と分かる必要がある"),
            new Temp("DuckSlotOn",  58.0f, 1.000f, "アヒルが入った受け皿。点灯して残り何個かが分かる"),

            // ⚠️ room8 は通り抜けるだけの部屋で、中身は金属ロッカーばかり。
            //    全部が同じ冷たさだとサーモ役の画面が「灰色の塊が並んだ静止画」になり、
            //    見るものが無い。通路として通るだけの部屋こそ、待っている間に
            //    眺めるものが要る。一部に余熱を持たせ、ThermalBodyDrift で
            //    ゆっくり脈打たせて「何かが最近まで入っていた」と読ませる。
            new Temp("LockerWarm",  27.5f, 0.780f, "room8 の一部のロッカー。余熱。冷たい金属の中で拾える程度に留める"),
            new Temp("PipeHot",     46.0f, 0.980f, "room8 の配管。熱源としてはっきり読ませ、部屋の骨格を示す"),
