...
namespace Adobe.SubstanceEditor
{
    ...
    internal sealed class SubstanceEditorEngine : ScriptableSingleton<SubstanceEditorEngine>
    {
        ...
        /// </summary>
        private readonly Dictionary<string, SubstanceNativeGraph> _activeSubstanceGraphcs = new Dictionary<string, SubstanceNativeGraph>();
        ...
        private readonly Queue<string> _delayedInitilization = new Queue<string>();
        ...
        {
            while (_delayedInitilization.Count != 0)
            {
                var instancePath = _delayedInitilization.Dequeue();
        ...
        {
            var runtimeGraphcsBehavior = FindObjectsByType<Substance.Runtime.SubstanceRuntimeGraph>(FindObjectsSortMode.None);
        ...
        public void InitializeInstance(SubstanceGraphSO substanceInstance, string instancePath, out SubstanceGraphSO matchingInstance)
        {
            ...
            if (!_activeSubstanceGraphcs.TryGetValue(substanceInstance.GUID, out SubstanceNativeGraph _))
            {
                var substanceArchive = Engine.OpenFile(substanceInstance.RawData.FileContent, substanceInstance.GetNativeID());
                _activeSubstanceGraphcs.Add(substanceInstance.GUID, substanceArchive);
            }

            if (!string.IsNullOrEmpty(instancePath))
                _delayedInitilization.Enqueue(instancePath);
            else
            {
        ...
        public void ReleaseInstance(SubstanceGraphSO substanceInstance)
        {
            ...
            {
                _activeSubstanceGraphcs.Remove(substanceInstance.GUID);
        ...
        public bool TryGetHandlerFromInstance(SubstanceGraphSO substanceInstance, out SubstanceNativeGraph substanceHandler)
        {
            ...
            if (!_activeSubstanceGraphcs.TryGetValue(substanceInstance.GUID, out substanceHandler))
            {
                return false;
            }
        ...
        {
            if (!_activeSubstanceGraphcs.TryGetValue(guid, out substanceHandler))
                return false;
        ...
        public void DisableAllRuntimeOnly()
        {
            var guids = AssetDatabase.FindAssets(string.Format("t:{0}", typeof(SubstanceGraphSO)));
            bool hasChangedGraph = false;

            for (int i = 0; i < guids.Length; i++)
            {
                string assetPath = AssetDatabase.GUIDToAssetPath(guids[i]);
                SubstanceGraphSO substanceInstance = AssetDatabase.LoadAssetAtPath<SubstanceGraphSO>(assetPath);

                if (substanceInstance.IsRuntimeOnly)
                {
                    substanceInstance.IsRuntimeOnly = false;
                    substanceInstance.OutputRemaped = true;
        ...
        {
            if (SubstanceEditorSettingsSO.DisableRuntimeOnly())
                return;
